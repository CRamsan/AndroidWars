package com.cesarandres.aw.util.astart;
...
public class AStarPathFinder {
	 ...
	 */
	public Path findPath(int maxDistance, int sx, int sy, int tx, int ty) {
		// easy first check, if the destination is blocked, we can't get there
		if (!this.isValidLocation(tx, ty)) {
			return null;
		} else {
			// If the distance of a straight line draw from the origin to the
			// destination tile
			// is longer than the maxDistance to move, we can't get there.
			if (Math.abs(tx - sx) > maxDistance
					|| Math.abs(ty - sy) > maxDistance
					|| Math.abs(tx - sx) + Math.abs(ty - sy) > maxDistance) {
				return null;
			}
		}

		// The initial state for A*. The closed group is empty. Only the
		// starting
		...
		while ((open.size() != 0)) {
			...
			for (int i = 0; i < 4; i++) {
				...
				// The neighbor tile will only evaluated if is a valid place and
				// if is not outside of the movement range
				// This last check will be done again.
				if (isValidLocation(Nx, Ny)
						&& (Math.abs(tx - Nx) + Math.abs(ty - Ny) <= maxDistance)) {
					// the cost to get to this node is cost the cost to reach
					// this node.
					Node neighbour = nodes[Nx][Ny];
					int nextStepCost = (int) (neighbour.cost + current.depth);

					// If the current evaluation finds that this movement have
					// less cost than
					// previously evaluated, then the tile can be evaluated
					// again to find a better path.
					...
					if (!inOpenList(neighbour) && !(inClosedList(neighbour))) {
						neighbour.heuristic = (float) Math
								.sqrt((currentDistanceX * currentDistanceX)
										+ (currentDistanceY * currentDistanceY));
						// If moving to the neighbor tile doenst exceeds our
						// movement range, then can be added.
						// This check is done twice in case this tile has been
						// reevaluated previously for a better path.
	...
	/**
	 * Find all the tiled that can be reached with the current parameters,
	 * mover(unit), max distance and location.
	 * 
	 * @param mover
	 ...
	 * @param maxDistance
	 *            The max distance the mover will move before running out of
	 *            stamina
	 * @param sx
	 ...
	 */
	public ArrayList<Node> findRange(int maxDistance, int sx, int sy,
			boolean isMovekRange) {
		...
		while ((open.size() != 0)) {
			...
			for (int i = 0; i < 4; i++) {
				...
				// if is not outside of the movement range
				// This last check will be done again.
				if ((isMovekRange && (isValidLocation(Nx, Ny) && (Math.abs(sx
						- Nx)
						+ Math.abs(sy - Ny) <= maxDistance)))
						|| (!isMovekRange && (Math.abs(sx - Nx)
								+ Math.abs(sy - Ny) <= maxDistance))) {
					// the cost to get to this node is cost the cost to reach
					// this node.
					Node neighbour = nodes[Nx][Ny];
	...
	protected boolean isValidLocation(int x, int y) {
		...
		boolean invalid = (x < 0) || (y < 0)
				|| (x >= nodes.length || (y >= nodes[0].length));

		// If is inside the limits of the map, will be checked if the tile is
		// aviable for use.
		if (!invalid) {
			invalid = !this.map.isEmpty(x, y);
		}
